...
//            XuToast.show(AppContext.getInstance(),"写出的数据："+new String(Hex.encodeHex(characteristic.getValue())));
    ...
    /**
     *开启扫描(只扫描有特定UUID服务的设备)
     * @param serviceUuids:服务的UUID数组
     * @param resultCallback:扫描结果的calllback
     */
    public boolean startScan(UUID[] serviceUuids,BleScanResultCallback resultCallback) {
        this.resultCallback = resultCallback;
        try {
            return mBtAdapter.startLeScan(serviceUuids,mLeScanCallback);
        } catch (Exception e) {
            e.printStackTrace();
            Log.d(TAG, "使用BLE扫描API开始扫描出错了 错误:" + e.getMessage());
            return false;
    ...
    private  class OperationTask extends AsyncTask<BleRequest, Void,String> {
        @Override
        ...
        @Override
        protected String doInBackground(BleRequest... params) {
            //在这里进行操作
            Operation(params[0]);
            return "";
        }
